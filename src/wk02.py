...
)  # .lower() is only for verifying if its a he or a she
...
total_commission = (share_count * share_price * sb_commission) + (
    share_count * current_stock_price * sale_commission
)
print(f"Total commission = {total_commission}")
print(
    f"Net profit / loss = {(share_count * current_stock_price) - (share_count * share_price) - total_commission}"
)
